...
export async function mountStage(container, opts) {
  ...
  function frame(now) {
    ...
    /* Свёрнутая вкладка не рисует. Плоская сцена так и делала с самого
       начала, объёмная — нет: браузер на телефоне продолжает выдавать кадры
       ещё какое-то время после сворачивания, и всё это время устройство
       считает полный проход WebGL с тенями и затенением. Телефон в кармане
       греться не должен, а счётчик кадров после возврата не должен видеть
       один огромный интервал и решать, что железо слабое. */
