...
function startGame(room) {
  ...
  /* Сюжет, не рассчитанный на собравшийся стол, движок меняет сам — и раньше
     делал это молча. Хозяин выбирал «Колоду из ломбарда» на десять человек,
     садилось двенадцать, и партия начиналась совсем другой историей без
     единого слова о том, куда делась выбранная. С пресетом ролей об этом
     давно говорят в общем чате; сюжет ничем не хуже. */
  if (sc && !fits) {
    room.chat.push({
      system: true, ts: Date.now(),
      text: 'Сюжет «' + sc.title + '» рассчитан на ' + sc.min + '–' + sc.max +
        ' человек, а за столом ' + members.length + ' — берём другую историю.'
    });
  }
